...
//            clearHandles();
//            //new line
//            editLine.invalidateLengthCache(); // or keep them if you want persistent dots
//            return;
//        }
...
public class ConnectionController extends MouseInputAdapter {
    ...
    private BendPoint dragBend = null;  // last-added bend to drag (handles)
    ...
    private Point midStartOnDrag = null; // original middle point when we start dragging
    private int   lenBeforeDrag  = 0;       // line length before the drag
    ...
    private void initLinePopup() {
        ...
        remove.addActionListener(e -> {
            ...
            editLine.getStart().setLine(null);
            editLine.getEnd().setLine(null);
            // remove from model (also subtracts length)
            model.removeLine(editLine);
            editLine = null;
            mode = Mode.IDLE;
            clearHandles();
        ...
        center.addActionListener(e -> {
            ...
            editLine.addChangeCenter(lastContextPoint);   // adds a 20s trigger point
            canvas.repaint();
        });
        ...
        zeroAcc.addActionListener(e -> {
            ...
            editLine.addZeroAccelPoint(lastContextPoint); // adds a 20s trigger point
            canvas.repaint();
        });
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (mode == Mode.DRAG_MIDDLE) {
            ...
            if (delta > 0 && !model.canAffordDelta(delta)) {
                ...
                if (delta != 0) model.applyWireDelta(delta);
                editLine.invalidateLengthCache();
            }

            mode = Mode.IDLE;
            clearHandles();            // or keep them if you prefer
            canvas.repaint();
            return;
        }
        ...
        if (dragSource != null) {
            ...
            if (target != null && target.getType() == dragSource.getType()) {
                ...
                if (!model.canCreateWire(dragSource, target)) {
                    JOptionPane.showMessageDialog(canvas,
                    ...
                    dragSource.setLine(wire);
                    target.setLine(wire);
                    model.addLine(wire); // updates used length
                }
            }
            dragSource = null;
            canvas.repaint();
    ...
    // Global budget check via delta (works with or without helper methods on SystemManager)
    private boolean canAffordDelta(int delta) {
        // If you already have model.canAffordDelta(delta), just call that and delete this method.
        return model.getWireUsedPx() + delta <= model.getWireBudgetPx();
    }
}
